...
@php
    $isPublicInitial = (bool) old('is_public', isset($weatherStation) ? $weatherStation->is_public : false);
    $apiEnabledInitial = (bool) old('api_enabled', isset($weatherStation) ? $weatherStation->api_enabled : false);
    $hasTokenInitial = isset($weatherStation) && $weatherStation->api_token;
@endphp
<form method="POST"
    ...
    <div class="border-b border-dark-border/50">
        <nav class="flex space-x-1" aria-label="Tabs">
            <button type="button"
                    ...
                    class="px-4 py-3 border-b-2 font-medium text-sm transition-colors flex items-center space-x-2 border-netatmo-purple text-netatmo-purple">
            ...
            @if(isset($weatherStation))
            <button type="button"
                    data-tab-button="sharing"
                    class="px-4 py-3 border-b-2 font-medium text-sm transition-colors flex items-center space-x-2 border-transparent text-purple-400 hover:text-purple-300 hover:border-purple-500/30">
    ...
    <div class="min-h-[400px]">
        ...
        <div data-tab-panel="general" class="space-y-6">
        ...
        <!-- Public Sharing Tab -->
        @if(isset($weatherStation))
        <div data-tab-panel="sharing" class="hidden space-y-6">
            <div class="bg-purple-900/20 border border-purple-700/30 rounded-xl p-6">
                <div class="flex items-start justify-between space-x-4">
                    ...
                    <!-- Toggle Switch -->
                    <button type="button"
                            data-switch="is_public"
                            data-switch-on-class="bg-purple-500"
                            data-switch-state="{{ $isPublicInitial ? 'on' : 'off' }}"
                            class="relative inline-flex h-8 w-14 items-center rounded-full transition-colors flex-shrink-0 {{ $isPublicInitial ? 'bg-purple-500' : 'bg-gray-600' }}">
                        <span data-switch-knob
                              class="inline-block h-6 w-6 transform rounded-full bg-white transition-transform {{ $isPublicInitial ? 'translate-x-7' : 'translate-x-1' }}"></span>
                    </button>
                    <input type="hidden" name="is_public" value="{{ $isPublicInitial ? '1' : '0' }}">
                </div>

                <div class="mt-4 {{ $isPublicInitial ? '' : 'hidden' }}" data-switch-show="is_public">
                        ...
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button type="button"
                                            data-copy-text="{{ route('netatmo.public', $weatherStation->uuid) }}"
        ...
        <!-- API Access Tab -->
        @if(isset($weatherStation))
        <div data-tab-panel="api" class="hidden space-y-6"
             data-token-manager
             data-has-token="{{ $hasTokenInitial ? '1' : '0' }}">
            <div class="bg-blue-900/20 border border-blue-700/30 rounded-xl p-6">
                <div class="flex items-start justify-between space-x-4">
                    ...
                    <!-- Toggle Switch -->
                    <button type="button"
                            data-switch="api_enabled"
                            data-switch-on-class="bg-blue-500"
                            data-switch-state="{{ $apiEnabledInitial ? 'on' : 'off' }}"
                            class="relative inline-flex h-8 w-14 items-center rounded-full transition-colors flex-shrink-0 {{ $apiEnabledInitial ? 'bg-blue-500' : 'bg-gray-600' }}">
                        <span data-switch-knob
                              class="inline-block h-6 w-6 transform rounded-full bg-white transition-transform {{ $apiEnabledInitial ? 'translate-x-7' : 'translate-x-1' }}"></span>
                    </button>
                    <input type="hidden" name="api_enabled" value="{{ $apiEnabledInitial ? '1' : '0' }}">
                </div>

                <div class="mt-4 {{ $apiEnabledInitial ? '' : 'hidden' }}" data-switch-show="api_enabled">
                    <!-- API Token Section -->
                    <div class="bg-dark-surface/40 border border-blue-700/20 rounded-lg p-4 space-y-4">
                            <div>
                                ...
                                <div data-token-status class="{{ $hasTokenInitial ? '' : 'hidden' }} flex items-center space-x-2 mb-3">
                                ...
                                <div data-token-set class="{{ $hasTokenInitial ? 'hidden' : '' }} mb-3">
                                    <button type="button"
                                            data-token-action="change"
                                ...
                                <div data-token-remove-confirm class="hidden space-y-3 mb-3">
                                ...
                                <!-- Token Input Form -->
                                <div data-token-input class="hidden space-y-3">
                                    <!-- Warning if changing existing token -->
                                    <div data-token-change-warning class="{{ $hasTokenInitial ? '' : 'hidden' }} bg-yellow-900/20 border border-yellow-700/40 rounded-lg p-3">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <button type="button"
                                                    data-token-action="cancel-input"
            ...
            <div class="bg-green-900/20 border border-green-700/30 rounded-xl p-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Authentication Info -->
                    <div data-token-show-when-set class="{{ $hasTokenInitial ? '' : 'hidden' }} bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-4">
                        <div class="flex items-start space-x-3">
                    ...
                    <div data-token-show-when-unset class="{{ $hasTokenInitial ? 'hidden' : '' }} bg-blue-900/20 border border-blue-700/30 rounded-lg p-4">
                        <div class="flex items-start space-x-3">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <div data-token-show-when-set class="{{ $hasTokenInitial ? '' : 'hidden' }}">
                                    ...
                                    <div data-token-show-when-unset class="{{ $hasTokenInitial ? 'hidden' : '' }}">
